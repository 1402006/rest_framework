...
export function ServiceButton({ service, onSelect, disabled }: Props) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className="flex flex-col items-center gap-3 rounded-2xl border border-border
                 bg-surface-raised px-4 py-8 text-center
                 transition-colors hover:border-accent hover:bg-accent-soft
                 focus-visible:outline focus-visible:outline-offset-2
                 focus-visible:outline-accent
                 disabled:pointer-events-none disabled:opacity-50"
    >
      {Icon && <Icon size={32} strokeWidth={1.75} className="text-accent" aria-hidden="true" />}
