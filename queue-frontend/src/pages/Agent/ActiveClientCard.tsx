...
function InfoRow({ icon: Icon, label, value }: { icon: typeof Phone; label: string; value: string }) {
  return (
    <div className="flex items-start gap-2.5">
      <Icon size={16} className="mt-0.5 text-text-muted" aria-hidden="true" />
      <div>
  ...
  return (
    <div className="rounded-2xl border border-accent bg-accent-soft p-6">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-text-muted">Client en cours</p>
          <p className="text-3xl font-semibold text-text">{ticket.ticket_code}</p>
          <p className="text-sm text-text-muted">{ticket.service.service_description}</p>
        </div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 rounded-xl bg-surface-raised p-4 sm:grid-cols-2">
      ...
      <div className="flex flex-wrap gap-2">
        {isCalled && (
          <>
            <button
              ...
              onClick={onStart}
              disabled={disabled}
              className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-border
                         bg-surface-raised px-3 py-2 text-sm font-medium text-text
        ...
        {isInProgress && (
          <button
            ...
            onClick={onComplete}
            disabled={disabled}
            className="flex items-center gap-1.5 rounded-lg bg-success px-3 py-2
      ...
      {otherGuichets.length > 0 && (
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
          <span className="text-xs text-text-muted">Transférer vers</span>
          <select
            value={transferTarget}
            onChange={(e) => setTransferTarget(Number(e.target.value))}
            className="rounded-lg border border-border bg-surface-raised
                       px-2 py-1.5 text-sm text-text"
          >
            {otherGuichets.map((g) => (
          ...
          <button
            ...
            disabled={disabled}
            className="flex items-center gap-1.5 rounded-lg border border-accent
